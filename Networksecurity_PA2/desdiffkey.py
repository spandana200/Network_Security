...
def makeplot(key, plaintxt):
    df = pd.DataFrame()
    for i in range(1,6):
        r1,rounds1 = d.encrypt(key[0], plaintxt)
        r2,rounds2 = d.encrypt(key[i], plaintxt)
        y = helperhd(rounds1, rounds2, i)
        df['HD'+str(i)] = y
    df = df.T
    boxplot = df.boxplot()
    boxplot.set_title("Avalanche effect with different keys",fontweight="bold")
...
# plaintext
plaintxt = '02468aceeca86420'
# Distict keys 
key = ['0f1571c947d9e859', '0f1571c947d9e850', '0a1571c947d9e859', '0f1571c047d9e859', '0f1571c947d9e059', '0f1579c947d9e859']
