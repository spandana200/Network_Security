...
def makeplot(key, plaintxt):
    df = pd.DataFrame()
    for i in range(1,6):
        r1,rounds1 = d.encrypt(key, plaintxt[0])
        r2,rounds2 = d.encrypt(key, plaintxt[i])
        y = helperhd(rounds1, rounds2, i)
        df['HD'+str(i)] = y
    df = df.T
    plot = df.boxplot()
    plot.set_title("Avalanche effect with different plaintexts",fontweight="bold")
    plot.set_xlabel("Rounds")
    plot.set_ylabel("Hamming Distances")
    print('Distinct Plain Text with same Key')
    print(plot)
...
# Distinct plain text with one Hamming Distance
plaintxt = ['02468aceeca86420', '02468adeeca86420', '12468aceeca86420', '02468aceeca06420', '02469aceeca86420', '02468aceeca85420']
        

d = des()

makeplot(key, plaintxt)
